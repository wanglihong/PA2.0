...
{    
    ...
    self.contentSize = CGSizeMake(self.frame.size.width, scrollHeight + LOADINGVIEW_HEIGHT + 46);
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    if (bottomEdge >= scrollView.contentSize.height ) {
        if ([_flowdelegate respondsToSelector:@selector(flowViewReachesBottom)]) {
            [_flowdelegate flowViewReachesBottom];
        }
    }
}
